...
def load_client_plugins(client, plugins):
    ...
    for plugin_name, module_name in plugins.items():
        log.debug(
            ...
            plugin_name,
            module_name,
# 🔒 VOTAL.AI Security Fix: Untrusted dynamic module import for plugins can lead to arbitrary code execution [CWE-829] - HIGH
        )
        try:
            spec = importlib.util.find_spec(module_name)
            if not spec or not spec.origin or not os.path.realpath(spec.origin).startswith(os.path.realpath(os.path.dirname(importlib.import_module("botocore.plugins").__file__))):
                raise ModuleNotFoundError(f"Disallowed plugin module: {module_name}")
